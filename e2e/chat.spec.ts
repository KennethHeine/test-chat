...
test("research tracker: summary shows open/researching/resolved counts", async ({ page }) => {
  ...
  await page.locator("#view-toggle-btn").click();
  await page.locator(".dashboard-nav-item[data-page='research']").click();
  await expect(page.locator(".research-page-summary")).toBeVisible({ timeout: 10_000 });

  // Summary should show counts for each status
  await expect(page.locator(".research-page-summary")).toContainText("Open: 1");
  await expect(page.locator(".research-page-summary")).toContainText("Researching: 1");
  await expect(page.locator(".research-page-summary")).toContainText("Resolved: 1");
});
...
test("milestone timeline page: summary line shows status counts", async ({ page }) => {
  ...
  await page.locator("#view-toggle-btn").click();
  await page.locator(".dashboard-nav-item[data-page='milestones']").click();
  await expect(page.locator(".milestone-page-summary")).toBeVisible({ timeout: 10_000 });

  // 1 draft, 0 ready, 1 in-progress, 1 complete
  await expect(page.locator(".milestone-page-summary")).toContainText("Draft: 1");
  await expect(page.locator(".milestone-page-summary")).toContainText("In Progress: 1");
  await expect(page.locator(".milestone-page-summary")).toContainText("Complete: 1");
});
...
test("issue draft page: summary line shows status counts", async ({ page }) => {
  ...
  await page.locator("#view-toggle-btn").click();
  await page.locator(".dashboard-nav-item[data-page='issues']").click();
  await expect(page.locator(".issue-page-summary")).toBeVisible({ timeout: 10_000 });

  await expect(page.locator(".issue-page-summary")).toContainText("Draft: 1");
  await expect(page.locator(".issue-page-summary")).toContainText("Ready: 1");
  await expect(page.locator(".issue-page-summary")).toContainText("Created: 1");
});
